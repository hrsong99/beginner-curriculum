...
깔리는 것이지 글자를 칠하는 것이 아니고, 규칙이 무엇으로 갈리느냐에 따라 위치가 다릅니다 —
받침으로 갈리면 `.on`/`.off`(바닥), 마지막 모음으로 갈리면 `.vowel`(오른쪽 세로획).
글자를 그대로 두고 캡션에만 「ㅏ · ㅗ」라고 적으면, 정작 어디를 보라는 건지가 화면에
없습니다. 바닥 자리를 모음에 쓰지 마세요 — 같은 덱 앞쪽에서 이미 받침을 뜻하고, ㅗ 계열
모음이 실제로 앉는 자리이기도 합니다.
