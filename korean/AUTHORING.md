...
- 빈칸이 낱말이 아니라 **문장 한 토막**이면(대화 채우기처럼 서술어 전체를 말하게 하는
  자리) `.slot wide` 로 둡니다. 앞말은 윗줄에 남고 칸이 아랫줄을 통째로 차지하며, 문장 끝
  부호만 옆에 붙습니다. **한 장 안에서는 다 같이 `wide` 로 하거나 다 같이 하지 않습니다** —
  들어가는 줄과 안 들어가는 줄을 눈으로 골라내면, 글꼴이 조금만 달라져도 그 판단이 뒤집혀
  칸 하나가 홀로 아랫줄로 밀려납니다. 한 줄에 넉넉히 들어가는 **낱말** 빈칸은 그냥 둡니다.
