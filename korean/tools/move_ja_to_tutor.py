...
Japanese-laden comes back unhandled rather than being emptied.
  ...
  python3 korean/tools/move_ja_to_tutor.py             # report
  python3 korean/tools/move_ja_to_tutor.py --apply
"""
import re
import sys
...
def main(argv):
    ...
    paths = [a for a in argv if not a.startswith("--")] or [
        str(p) for p in sorted((REPO / "korean/tracks").glob("**/lesson.html"))]
    nm = nu = 0
    for f in paths:
        p = Path(f)
