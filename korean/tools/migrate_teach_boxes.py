...
under REVIEW, and --apply refuses to run on a box it could not fully resolve.
  ...
  python3 korean/tools/migrate_teach_boxes.py                # propose, print
  python3 korean/tools/migrate_teach_boxes.py --review       # + dropped text
  python3 korean/tools/migrate_teach_boxes.py --apply        # write the safe ones
  python3 korean/tools/migrate_teach_boxes.py --apply <deck…>
"""
import re
import sys
import json
...
def sentences(t, ja=False):
    """Split the way runtime/js/script-lines.js splits, for the same reasons.
...
def main(argv):
    ...
    if not paths:
        paths = [str(p) for p in sorted(
            (REPO / "korean/tracks/2-core-patterns").glob("**/lesson.html"))]
