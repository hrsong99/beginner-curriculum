...
- **A trial deck assumes the learner cannot read hangul, so every Korean they are asked to say carries a kana reading** (`.yomi`) — model lines, pattern cards, dialogue, the tiles they tap, the hint chips. Not the page title (its Japanese gloss already says what the page is), not the blue script box (the tutor reads that), not an answer key. This is the one place the deck deliberately overrides *romanize the parts*: that rule is about the hangul track, where decoding is the skill being built and a reading line would do the learner's work for them. A trial is often somebody's first hour of Korean and its job is to get them speaking, so the crutch is the point — and the **ア** switch in the pager takes it away the moment it stops being.
- **One switch, whole deck.** A reading toggle that lived per page would be twenty-five taps. It sets a class on `body`, so every page follows, and it is never shared to the other screen — how much of a crutch someone needs is theirs to decide.
