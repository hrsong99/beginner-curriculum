...
| Dialogue `.slot-input` at 360px | reproduced, not written | Long clause inputs in Core 20 `p3-complete` and `in-the-wild` extend up to 50px beyond their dialogue card. The static-input migration exposes `.slot-input`'s content-sized width inside narrow bubbles; this needs one shared responsive runtime rule, not per-deck markup or inline CSS |
