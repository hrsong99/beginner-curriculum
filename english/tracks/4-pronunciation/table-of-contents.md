...
As in every other track, **only sound travels** — the learner never sees the tutor. Model a sound by
*saying* it, and describe the **learner's own** mouth (`put your tongue between your teeth`), never
the tutor's.
...
Japanese is mora-timed: every mora takes about the same time. English is stress-timed: stressed
syllables carry the beat and everything between them compresses. Speaking English with even
timing sounds robotic and, worse, makes the stressed syllable impossible to find.
...
- **Perception before production.** Every lesson opens with listen-and-choose ("which one did you
  hear?"), then read, then say. A learner who cannot *hear* the difference between `right` and
  `light` will not be corrected into producing it, and the listening rung is the only step that
  proves the contrast has landed.
...
- **Lessons 1–3 overlap Core's vocabulary work**, but the job is different: Core teaches the word,
  this track teaches the shape of it. Do not re-teach the word; assume it and train the mouth.
