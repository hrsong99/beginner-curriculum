# Core Coverage and Disposition Audit

**Status:** targeted recommendations applied; affected rows require native re-review  
**Audited:** 2026-08-13  
**Applied:** 2026-08-13  
**Scope:** 122 Core lessons; stable IDs preserved

## Verdict

The pre-change Core spine was broadly coherent, but not ready to freeze unchanged. The audit found:

- **3 high-value beginner functions absent as primary Core targets;**
- **1 important function only partially covered;**
- **3 exact duplicate pattern frames;**
- **2 advanced lessons that should probably be demoted from primary production;** and
- **1 pair of lessons that the TOC itself says should be one contrastive lesson.**

The accepted correction did not increase the 122-lesson total. It merged or replaced redundant
advanced material and used the recovered capacity for ability, activity preferences and invitations.

### Implementation outcome

- Core 18, 27 and 28 now own ability, activity preferences and invitations respectively.
- Core 76 now teaches both causatives contrastively; Core 77 now owns service follow-up.
- Core 79, 105 and 112 now teach completion estimates, linked change and tone repair instead of
  future perfect, negative-adverbial inversion and a risky disagreement formula.
- Core 85, 95 and 121 now have distinct discourse jobs; the three exact duplicate pattern strings
  identified below have been eliminated. Core 108's informal request frame was also made
  register-specific so the new Core 18 ability question did not create another exact duplicate.
- Cross-track references affected by these changes were re-derived. Native feedback still decides
  final wording, and the changed review snapshots must be reviewed again.

The findings below preserve the pre-change rationale and record what drove the accepted edits.

## 1. Pre-change disposition of the six evidence-priority lessons

| Lesson | Audit disposition | Why | Replacement or next check |
| --- | --- | --- | --- |
| **Core 76** `have something done` | **merge with 77** | Useful service function, but no direct Core Inventory item was found. More importantly, Core 77's own JP note says the two causatives should be paired and contrasted in one lesson. | One contrastive service/arrangement lesson; verify both forms in EGP or an authorized equivalent. |
| **Core 77** `get someone to do` | **merge with 76** | Separate treatment contradicts its own teaching rationale and spends two lessons on one Japanese `てもらう` contrast. | Preserve the contrast, not two lesson slots. |
| **Core 79** future perfect | **demote unless workplace evidence is strong** | The Core Inventory places future perfect at B2; the current band is B1+. The TOC itself calls the form low-frequency in speech and a likely demotion candidate. | Keep `by Friday` deadline language in Core 80 or 82; make future perfect optional grammar unless corpus/native review earns production. |
| **Core 84** diplomatic request | **keep provisionally** | The communicative need is strong and completes the politeness progression. The evidence gap concerns the exact exponent and band, not the need. | Native/corpus check `I was wondering if you could`; treat B1+ as provisional. |
| **Core 105** negative-adverbial inversion | **demote** | The Core Inventory associates this with C1, not the current B2 band. The TOC independently calls it low-frequency in speech and its strongest demotion candidate. | Keep `not only ...` as an expression or receptive item; do not require productive `Never have I ...`. |
| **Core 112** `With all due respect` | **make receptive or remove as a Core lesson** | Its own note says “receptive first” and warns that learner production can start a fight. That conflicts with Core's two-primary-production-pattern contract. | Move recognition to Contextual/Freetalking support; retain only if native review supplies a safe productive outcome. |

### Applied result

Core 76/77 were consolidated, and 79, 105 and 112 were replaced in place. The recovered capacity
was used for the missing beginner functions below without renumbering or increasing the spine.
Core 84 remains provisional pending evidence and native review.

## 2. Missing or materially incomplete high-value content

### Critical targets now added

| Missing function | Source evidence | Current coverage | Recommended action |
| --- | --- | --- | --- |
| **Ability and limitation** — `I can ___, but I can't ___` | Core Inventory A1, item 104 `Can/can't (ability)`, printed p. 44; the existing transformation map also specifies this exact question/answer pairing. | Previously absent as a primary target. | **Applied at Core 18.** |
| **Activity preferences** — `I like/love/hate ___ing` | Core Inventory A1 item 86 and A2 item 86b, printed pp. 44 and 48. | Previously absent as a primary pattern. | **Applied at Core 27.** |
| **Inviting and responding** — `Would you like to ___?` / `I'd love to, but ___` | Core Inventory A2 function 15 and verb-complement exponents, printed pp. 46 and 48; the transformation map names this exact pair. | Previously stopped at checking availability and arranging a time. | **Applied at Core 28.** |

### Important but may be Core support or Contextual ownership

| Partial area | Evidence | Current state | Recommendation |
| --- | --- | --- | --- |
| **Giving route directions** | Core Inventory A1 function `Directions`; A2 travel/service exponents include asking the way. | Core 11 asks where a place is and gives only a static location (`behind`). It does not teach route instructions. | Decide ownership explicitly. If Core owns it, teach `Go straight / turn ...`; otherwise name the Contextual prerequisite rather than claiming Core coverage. |
| **Basic possession and availability** | Core Inventory A1 examples repeatedly use `have/has`, `any/some`, possessions and family quantities. | Core 5 covers ownership with `mine`; later `have` is perfect, obligation or hypothetical. Ordinary `I have / Do you have` is not a primary target. | Add as grammar/expression support unless learner trials show it needs a full lesson. Do not automatically spend a slot. |

## 3. Duplicate and competing lessons

### Exact duplicate pattern strings found and resolved

| Pattern | Lessons | Audit |
| --- | --- | --- |
| `I don't ___.` | Core 14 and former Core 27 | Core 27 was rewritten around activity preferences. |
| `I'm not sure ___.` | former Core 18 and Core 85 | Core 18 now owns ability; Core 85 received a more specific advanced second move. |
| `Let's just say ___.` | Core 119 and former Core 121 | Core 119 retains deliberate implication; Core 121 now sets an explicit conversational boundary. |

### Near-duplicate lesson functions

- **Core 85 and 95** both teach acknowledge-then-disagree (`I see your point, but` / `I take your
  point, but`). The B2 version needs a materially different discourse job or the pair should merge.
- **Core 93 and 115** both teach concede-then-counter. A level progression may justify both, but the
  corpus audit must show that Core 115 adds more than elevated vocabulary.
- **Core 119 and 121** are not merely related: they share a primary pattern and both withhold detail.
  One lesson should own this move.

Graded recurrences such as Core 20→60→84→108 requests, Core 21→61 permission, and Core 57→66
conditionals are not duplicates. Their communicative pressure or form changes materially at each
stage.

## 4. Inventory items that should not automatically become lessons

The inverse comparison also found source items absent from primary patterns that should remain
support, lexical metadata or deliberate exclusions:

- **Possessive `’s`, question tags and phrasal-verb splitting:** useful forms, but not independent
  communicative outcomes. Carry them in grammar support, expressions or vocabulary provenance.
- **Zero conditional:** add as support beside Core 57 if needed; it does not automatically deserve a
  separate two-pattern lesson.
- **Extended passive range and mixed conditionals:** valid B2/C1 language, but require a real spoken
  can-do before occupying Core slots.
- **Future perfect continuous:** keep excluded. The curriculum's existing boundary and Cambridge's
  public EGP discussion both identify it as low-frequency; source presence alone is not a reason to
  teach it.

## 5. Level and endpoint finding

Individual C1-associated expressions do not demonstrate C1 proficiency. Core 108–122 supplies
useful register, stance and implication language, but the CEFR's C1 claims concern flexible,
precise and sustained performance as well as possession of exponents. Until representative lessons
and recorded performance exist, describe this section as a **C1 bridge / C1-associated language**,
not evidence that completing Core produces a C1 speaker.

This is a product-claim correction, not necessarily a reason to delete Units 20–22.

## 6. Applied change list and remaining decisions

### Critical

1. **Applied:** merge the Core 76 causatives and repurpose Core 77.
2. **Applied:** resolve the exact duplicate `Let's just say ___` in Core 119/121.
3. **Applied:** add ability, activity preferences and invitations using recovered capacity.
4. **Applied:** replace Core 105's inversion target with a spoken linked-change target.

### Important

5. **Applied:** replace Core 112's risky formula with impact-and-intent repair.
6. Decide whether Core or Contextual owns route directions.
7. **Applied:** rewrite Core 27 and the second pattern of Core 85.
8. **Partly applied:** Core 95 now identifies a specific argumentative weakness; Core 115 still
   needs corpus/native evidence that its concede-counter move adds enough value.

### Optional / evidence-led

9. **Applied:** replace Core 79's future perfect with a workplace completion estimate; verify the
   replacement frames in corpus/native review.
10. Add ordinary possession only if representative lessons expose a real production gap.
11. Relabel the endpoint conservatively until performance evidence supports a C1 claim.

## Sources and method

- [EAQUALS/British Council Core Inventory for General English](https://www.eaquals.org/wp-content/uploads/EAQUALS_British_Council_Core_Curriculum_April2011.pdf), especially the essential guide on printed pp. 12–13 and exponents on printed pp. 42–71.
- [CEFR Companion Volume (2020)](https://rm.coe.int/common-european-framework-of-reference-for-languages-learning-teaching/16809ea0d4), especially oral production pp. 62–65, oral interaction pp. 72–81 and sociolinguistic/pragmatic competence pp. 136–142.
- [`core-evidence-ledger.md`](./core-evidence-ledger.md) for the forward lesson→source mapping.
- [`linked-curriculum-map.md`](./linked-curriculum-map.md) for owner-supplied source transformation claims.
- [`jp-l1-difficulty-inventory.md`](./jp-l1-difficulty-inventory.md) for Japanese-L1 weighting.

Method: parse all 122 live Core rows; compare existing functions and forms against the Core
Inventory in reverse; check exact duplicate pattern strings mechanically; inspect near-duplicates,
internal rationale contradictions and CEFR-scale fit manually. Missing inventory labels were not
treated as missing lessons unless they represented a real speaking outcome for this learner.
