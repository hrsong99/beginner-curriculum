...
| 28 | ask and negate past action | `did`; `didn't`; base verb after auxiliary | A2 | 26–27 | all narrative contexts |
| 29 | sequence a story | `first/then/in the end`; coordinated past clauses | A2 | 26–28 | S2, T2 |
| 30 | add past background | past continuous; `when/while`; interrupted action | A2 | 26–29 | S2, T2 |
| 31 | state intention | `be going to`; future-time expression | A2 | 12, 19 | S1, T1 |
| 32 | state and check arrangement | present continuous for future; `still`; date/time | A2 | 12, 17 | S1, W1 |
| 33 | decide and promise | `will`; contraction `I'll`; deadline `by` | A2 | 20 | T1, W1 |
| 34 | express possibility | modal `might`; conditional response chunk | A2 | 21, 33 | T1, T2 |
| 35 | advise and state need | `should`; `need to`; simple condition | A2 | 19, 21 | S2, T2 |
| 36 | explain rules | `have to`; `don't have to`; `mustn't` | A2 | 21, 23 | E1, T1, W1 |
| 37 | describe an item | adjective + noun; indefinite pronoun `something + adjective` | A2 | 3–5 | E2, T2 |
| 38 | express degree | intensifiers; `a little`; contrast | A2 | 37 | E2, S2 |
...
| 46 | state continuing duration | present perfect; duration with `for`; `how long` | A2 | 43 | S2, W1 |
| 47 | explain ongoing recent activity | present perfect continuous; `that's why` | B1 | 12, 46 | S2, W1 |
| 48 | give a progress update | present perfect; fractions/quantities; `still need to` | B1 | 15, 44 | W1, W2 |
| 49 | connect reason and result | `because`; `so`; clause order | A2 | 26, 42 | all contextual courses |
| 50 | concede and contrast | `but`; `although`; subordinate clause | B1 | 38–42, 49 | S2, W2 |
| 51 | make a real condition | first conditional; present in `if` clause | A2 | 31–34 | T1, S1, W1 |
| 52 | state time and negative conditions | future time clause `when`; `unless` | B1 | 51 | T1, W1 |
...
| 60 | relay information/instruction | reported statement; `tell + object + to` | B1 | 25–28, 53 | S2, W1, W2 |
