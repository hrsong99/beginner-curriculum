...
writes course.yaml only and never creates a deployable lesson.yaml.
    ...
    python3 english/tools/plan_courses.py --all
    python3 english/tools/plan_courses.py --all --dry-run
"""
...
def course_yaml(course: dict, country_code: str | None = MARKET_COUNTRY_CODE) -> str:
    plan = "\n".join(
...
spec:
  ...
  title:
    # grape requires ko as its admin fallback even when LANG_TYPE is EN.
    ko: {yaml_str(course['title'])}
    en: {yaml_str(course['title'])}

  description:
    ko: {yaml_str(course['description'])}
    en: {yaml_str(course['description'])}
...
{plan}
# Generated by english/tools/plan_courses.py — edit the TOC/mapping and re-run.
"""
...
def generate(track: str | None, dry_run: bool) -> int:
    ...
    for course in courses:
        ...
        if not dry_run:
            ...
            target.write_text(course_yaml(course), encoding="utf-8")
